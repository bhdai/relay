...
``MCPClient`` extends ``MultiServerMCPClient`` with:

- Lazy tool loading via cached schemas (fast startup).
- Stateful session management for servers that need persistent
  connections.
- Tool filtering through include/exclude rules.
- Disk-based schema caching with config-hash invalidation.
"""
...
from relay.mcp.cache import MCPCache
from relay.mcp.loader import MCPLoader
...
class MCPClient(MultiServerMCPClient):
    ...
    def module_map(self) -> dict[str, str]:
        """Tool-name → server-name mapping for pattern matching."""
    ...
    async def _populate_live(self, server: str) -> None:
        ...
        if meta and meta.stateful:
            ...
            raw = await self._loader.stateless(server)
        self._live[server] = {t.name: t for t in raw}
