...
The registry tracks which tools have been registered from each server
and enforces include/exclude filters.  It also builds the module map
that enables ``mcp:server:tool_name`` pattern matching.
"""
...
class MCPRegistry:
    ...
    def register(self, name: str, server: str) -> bool:
        """Register tool. Returns True if newly registered, False if duplicate."""
        key = (server, name)
